class Enterprise::HumanResources::Attendance::AttendanceRecordsController < GenericResourceController
  ...
  def process_form(my_attendance_record, current_params, wizard_mode = nil)
    ...
    attendance_records_processing = Proc.new do
      ...
      my_attendance_record.implemented_on = current_params[:implemented_on]
  ...
  def validate_overlap
    ...
    implemented_on_time_in = DateTime.strptime(params[:date_time_in], '%Y-%m-%dT%H:%M')
    implemented_on_time_out = DateTime.strptime(params[:date_time_out], '%Y-%m-%dT%H:%M')
    ...
    AttendanceRecord.all.where(employee_id: employee_id).each do |att_rec|
      ...
      if ((implemented_on_time_in..implemented_on_time_out).overlaps?(other_date_time_in..other_date_time_out))
