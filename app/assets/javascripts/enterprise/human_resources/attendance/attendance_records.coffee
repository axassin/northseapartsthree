...
$( document ).ready( ->
  $('#new_attendance_record').submit( (event) ->
    ...
    date_of_attendance_form = $('#attendance_record_date_of_attendance').parsley()
    window.ParsleyUI.removeError(date_of_attendance_form, "overlap_error");
    ...
    $.ajax "attendance_records/validate_overlap?date_time_in=" + time_in + "&date_time_out=" + time_out + "&employee_id=" + employee_id,
      ...
      error: (jqXHR, textStatus, errorThrown) ->
        window.ParsleyUI.addError(date_of_attendance_form, "overlap_error", 'Overlaps with a previous Attendance Record')
